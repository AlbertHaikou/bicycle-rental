        ...
        <tr>
            ...
            <c:if test="${sessionScope.user.role eq 'ADMINISTRATOR'}">
                <td><a href="main?command=showEditManagerPage&id=<c:out value="${support.id}"/>"><fmt:message
                        key="UPDATE"/></a></td>
                <td><a href="main?command=deleteManager&id=<c:out value="${support.id}"/>"><fmt:message
                        key="DELETE"/></a></td>
                <td><a href="main?command=withdrawPrivileges&id=<c:out value="${support.id}"/>"><fmt:message
